	...
	public class UICheatsListItem : UIElement
	{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			Utils.DrawSettingsPanel(spriteBatch, pos, dimensions.Width + 1f,
				IsMouseHovering ? _color : _color.MultiplyRGBA(new Color(180, 180, 180)));

			pos.X += 8f;
			pos.Y += 8f;

			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.Value, _cheat.Name, pos,
				drawColor, 0f, Vector2.Zero, defScale, dimensions.Width + 1f);

			string text = _cheat.IsEnabled
				? Language.GetTextValue("UI.CheatEnabled")
				: Language.GetTextValue("UI.CheatDisabled");
